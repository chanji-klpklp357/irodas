...
class Featured_Image_Generator_Admin {
	...
	{
		echo "<div class='fig-inside gutenburg'><p class='fig_call_popup_container' style='margin: 0px;'><a href='#' class='fig_call_popup'>Featured Image Generator</a></p></div>";
	}

	public static function fig_add_custom_meta_box()
	{
		add_meta_box('fig-meta-box', 'FIG', array( 'Featured_Image_Generator_Admin', 'custom_meta_box_markup'), 'post', 'side', '', null);
	}
	...
	public static function init_hooks() {	
		...
		add_action( 'admin_init', array( 'Featured_Image_Generator_Admin', 'clear_temp_folder') );
		...
		// Meta Box
		add_action( 'add_meta_boxes', array( 'Featured_Image_Generator_Admin', 'fig_add_custom_meta_box') );
	}
	...
	public static function fig_setting_fields(){
		...
		// add_settings_field("fig_license_key", "License Key <label class='label--pro'>PRO</div>", array("Featured_Image_Generator_Admin", "display_license_key_element"), "theme-option", "section");
		...
		add_settings_field("fig_clear_image", "Clear Temporary Folder", array("Featured_Image_Generator_Admin", "display_clear_image"), "theme-option", "section");
		add_settings_field("fig_pixabay_api", "Pixabay API <label class='label--pro'>PRO</div>", array("Featured_Image_Generator_Admin", "display_pixabay_api_element"), "theme-option", "section");
		add_settings_field("fig_pixabay_safesearch", "Pixabay Safesearch <a href='https://pixabay.com/api/docs/'>?</a> <label class='label--pro'>PRO</div>", array("Featured_Image_Generator_Admin", "display_pixabay_safesearch"), "theme-option", "section");
		add_settings_field("fig_post_type", "Enable <label class='label--pro'>PRO</div>", array("Featured_Image_Generator_Admin", "display_post_type_element"), "theme-option", "section");		

		// register_setting("section", "fig_license_key");
    	...
    	register_setting("section", "fig_pixabay_api");
		register_setting("section", "fig_pixabay_safesearch");
	}

	/*public static function display_license_key_element(){
	?>
	<input type="text" name="fig_license_key" id="fig_license_key" placeholder="License Key" class="regular-text" value="" disabled="" />
    	<div class="fig_register_here">
    		<a href="https://designilcode.com/member/" target="_blank">Get License Key</a>
    	</div>
	...
	public static function display_pixabay_api_element(){
	?>
		<input type="text" name="fig_pixabay_api" id="fig_pixabay_api" placeholder="API" class="regular-text" value="<?php echo get_option('fig_pixabay_api'); ?>" disabled/>
		<div class="fig_register_here">
			<a href="https://pixabay.com/en/accounts/login/?next=/api/docs/" target="_blank">Get your own API</a>
		</div>
	<?php		
	}

	public static function display_pixabay_safesearch(){
		$safesearch = get_option('fig_pixabay_safesearch');
	?>
		<input type="checkbox" name="fig_pixabay_safesearch" id="fig_pixabay_safesearch" class="regular-text" value="1" <?php if ( $safesearch == 1 ) echo 'checked'; ?> disabled/>
		<label>True</label>
	<?php		
	}
	...
	public static function display_post_type_element(){
		$fig_post_type = get_option('fig_post_type');
		$page_chk = '';

		if ( $fig_post_type ) {
			if ( in_array('page', $fig_post_type) ) 
				$page_chk = 'checked';
		}
		?>
			<ul>
				<li>
					<input type="checkbox" name="fig_post_type[]" class="regular-text" value="page" <?php echo $page_chk; ?> disabled/>
					<label>Page</label>
				</li>
		<?php
		$args = array(
	    ...
	    if ( $post_types ) {
		    foreach ( $post_types  as $pt ) {
		    	...
		    	if ( $fig_post_type ) {
					if ( in_array($pt->name, $fig_post_type) ) 
						$cpt_chk = 'checked';
				}
		    	?>
					<li>
						<input type="checkbox" name="fig_post_type[]" class="regular-text" value="<?php echo $pt->name; ?>" <?php echo $cpt_chk; ?> disabled/>
						<label><?php echo $pt->label; ?></label>
					</li>
		    	<?php
		    }
		    ?>
	...
	public static function display_clear_image(){
		$upload_dir = wp_upload_dir();
		$files = glob($upload_dir["basedir"]."/fig_uploads/*");
		$count_files = count($files);
		?>
		<input type="submit" name="clear" class="button-secondary" value="Clear"> <span></span>
		<div style="margin-top:5px;">Currently <?php echo $count_files; ?> Files in Temp Folder.</div>
		<div style="margin-top:5px;">* Delete files in Temp Folder. May cause "photo" that use in "template" not found.</div>
		<?php
	}

	public static function clear_temp_folder(){
	...
	public static function fig_save_after_generate_image()
	{
		...
		if ( true ) {
			...
			$media_id = media_handle_sideload( $file_array, $_POST['post_id'], '' ); 
